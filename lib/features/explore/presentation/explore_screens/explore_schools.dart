...
class ExploreSchool extends StatefulWidget {
  const ExploreSchool({super.key});

  @override
  State<ExploreSchool> createState() => _ExploreSchoolState();
}

class _ExploreSchoolState extends State<ExploreSchool> {
  late List<LocationModel> list = [];
  Future<List<LocationModel>> getSchool() async {
    try {
      final response = await http.get(Uri.parse(getSchoolList));
