...
class ExploreAdventure extends StatefulWidget {
  const ExploreAdventure({super.key});

  @override
  State<ExploreAdventure> createState() => _ExploreAdventureState();
}

class _ExploreAdventureState extends State<ExploreAdventure> {
  late List<LocationModel> list = [];
  Future<List<LocationModel>> getAdventure() async {
    try {
      final response = await http.get(Uri.parse(getTrekList));
